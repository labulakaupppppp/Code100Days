...
#### tcp的拥塞控制
首先需要设置一个拥塞窗口cwnd(防止网络发生阻塞而设置的窗口),以及一个慢开始门限 ssthresh。为ssth设置一个初始值，假设为16.cwnd的初始值一般为1.
1. 慢开始：cwnd=1之后每接受一个ack就将cwnd加倍。即cwnd变化为1 2 4 8 16 
2. 拥塞避免：当cwnd>=ssth时开始执行拥塞避免，即每次为cwnd+1，cwnd变化为：17 、18、....24,直到出现超时。
3. 出现超时则说明网络阻塞，需要重新执行慢开始，并且将ssth=cwnd/2;此时ssth=24/2=12.并将cwnd=1（每次慢开始，都将cwnd置为1）。cwnd变化为 1，2，4，8，12.
4. cwnd=ssth时开始执行拥塞避免 cwnd变化为13、14.....16.若此时出现连续收到多个ack则代表丢失了某报文，如连续3个报文2的ack，这代表报文3丢失。
5. 快重传：出现报文丢失，需要立刻重传报文3，即所谓的快重传。
6. 快恢复：出现报文丢失，并不代表网络阻塞，因为还收到了三个连续的ack，此时执行快恢复，直接将ssth=cwnd/2=16/2=8,继续执行拥塞避免。即cwnd变化为8、9、10.。。。。。
